...
def find_prediction(
    contains: List[AllergenPrediction], allergen: SupportedAllergen
) -> Optional[AllergenPrediction]:
...
def pair_expected_and_produced_items(expected: dict, produced: LabeledAllergenMenu):
    ...
    for section in produced.sections:
        for item in section.items:
            ...
            for contains_allergen, expected_prediction in expected[item_name].items():
                ...
                if not (
                    produced_prediction := find_prediction(
                        contains=item.contains, allergen=allergen
                    )
                ):
                ...
                results.append(
                    [
                        item_name,
                        allergen,
                        produced_prediction.prediction,
                        produced_prediction.explanation,
                        expected_prediction,
                    ]
                )
    ...
    return pd.DataFrame(
        results,
        columns=["Item Name", "Allergen Name", "Prediction", "Explanation", "Expected"],
    )
